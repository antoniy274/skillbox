...
void MatrixGraph::resize(size_t new_size)
{
    ...
    if (_graph != nullptr)
    {
        ...
        //”дал€ем старый массив из п€м€ти
    ...
    //—охран€ем новый размер
    _size = new_size;

    //увеличиваем емскость
    _capacity = new_size + c_ratio;

    //¬ыдел€ем новый размер массива из п€м€ти c увеличенной емкостью
    ...
    // опируем массив из временного в посто€нный
...
MatrixGraph::MatrixGraph(const MatrixGraph& list_g) : MatrixGraph()
{
    ...
    //¬ыдел€ем новый размер массива из п€м€ти
...
void MatrixGraph::AddEdge(size_t from, size_t to)
{
    ...
    //ѕроверка на длину ребра и если мало увеличиваем массив и емкость
    if (from >= _capacity || to >= _capacity)
    {
        //”величиваем размер на 1 из за принимаемых параметров от 0
        std::size_t max_from = std::max(_capacity, from + 1);
        std::size_t max_to = std::max(_capacity, to + 1);
        resize(std::max(max_from, max_to));
    }
    //ѕроверка на длину ребра и если мало увеличиваем рабочий размер массива
    ...
    //—оедин€ем вершины от from до to 
